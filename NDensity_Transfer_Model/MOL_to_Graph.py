# part 1/2
"""
build_data_from_mol_alignn.py

Implementation for constructing atomic graph and line graph data for ALIGNN.
The returned Data object contains:
 - x: atom features (N, node_feat_dim)
 - edge_index: atom graph edge indices (2, E_atom)
 - edge_attr: atom graph edge features (E_atom, edge_feat_dim)
 - line_graph_edge_index: line graph edge indices (2, E_line)
 - line_graph_edge_attr: line graph edge features (E_line, angle_feat_dim)
...
def compute_bond_angle_features(pos, edge_index, neighbors):
    ...
    # For each bond pair (sharing one atom), create angle features
    for e1_idx in range(n_edges):
        i, j = edge_index[:, e1_idx].tolist()

        # Find all edges connected to j (excluding current edge)
...
def build_alignn_data_from_mol(
    ...
    """
    Build ALIGNN data from an RDKit molecule.

    Args:
        mol: RDKit molecule (should contain 3D conformer)
    ...
    for atom in mol.GetAtoms():
        ...
        if total_electrons > 0:
            valence_ratio = valence / total_electrons
        else:
            valence_ratio = 0.0
    ...
    # ---------- Atom graph edges (chemical bonds) ----------
    row, col = [], []
    edge_features = []
    ...
    # Edges are already added in both directions, so do not call to_undirected()
    # ---------- Line graph edges ----------
    # In ALIGNN, nodes of the line graph are edges of the atom graph.
    # Edges represent bond pairs sharing a common atom.
    ...
    frac_N = num_N / n_atoms if n_atoms > 0 else 0.0
    u_N = torch.tensor([[num_N, frac_N]], dtype=torch.float32)
